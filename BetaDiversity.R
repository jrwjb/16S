...
count_phy <- otu_table(otu_table, taxa_are_rows = T)
...
plot(compute_pca, typeVc ="x-score", parAsColFcVn = factor(sample_info$Group), parCexN = 0.8, parCompVi = c(1, 2), 
     ...
     file.pdfC = paste0(path, '/03_Diversity/Beta/PCA/pca.pdf'),.sinkC = NULL) 
...
write.table(OUT1, paste0(path, '/03_Diversity/Beta/PCA/pca.txt'), sep = '\t',row.names = F)
...
png(filename = paste0(path, '/03_Diversity/Beta/PCA/pca.png'),width = 26,height = 22,units = "cm",res = 300)
if (length(unique(as.numeric(samDF[,2]))) >6){
...
ggsave(paste0(path, '/03_Diversity/Beta/NMDS/NMDS.png'), width = 25, height = 18, units ="cm", dpi = 600)
ggsave(paste0(path, '/03_Diversity/Beta/NMDS/NMDS.pdf'))
...
pcoa_func <- function(data, file){
    ...
    ggsave(paste0(out, '/PCoA/', index, 'PCoA.png'), width = 25, height = 18, units ="cm", dpi = 600)
    ggsave(paste0(out, '/PCoA/', index, 'PCoA.pdf'))
}

pcoa_func(weighted_unifrac, weighted_unifrac_file)
pcoa_func(unweighted_unifrac, unweighted_unifrac_file)
...
write.table(result_anosim, paste0(path, '/03_Diversity/Beta/anosim/anosim_result.txt'), sep = '\t', row.names = F)
write.table(result_adonis, paste0(path, '/03_Diversity/Beta/adonis/adonis_result.txt'), sep = '\t', row.names = F)
...
p_value <- anosim$signif
anosim_result <- data.frame(groupvs, R, p_value)
write.table(anosim_result, paste0(path, '/03_Diversity/Beta/anosim/', groupvs, '.txt'), sep = '\t', row.names = F)
png(filename = paste0(path, '/03_Diversity/Beta/anosim/', groupvs, '.png'), width = 26, height = 22, units = "cm", res = 300)
plot(anosim, col=cols)
dev.off()
#pdf
