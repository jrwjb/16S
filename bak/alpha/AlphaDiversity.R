...
otu_table <- read.table(paste0(path, '/02_OTU/otu_table.txt'), row.names = 1, header = T, sep = '\t', check.names = F)
...
for (index in index_list) {
    ...
    for (i in 1:nrow(groupVs)) {
        ...
        if (group_num == 2){
            ...
            ## 判断是否是正太分布
            normality_test <- shapiro.test(tmp_data$x)
            if (normality_test$p.value > 0.05){
            ## t-test
                ttest <- t.test(x ~ Group, tmp_data)  # 假设俩个变量方差不齐
                p_value <- ttest$p.value
                ttest_result <- data.frame(groupvs, p_value)
                ttest_result$method <- 't-test'
                result <- rbind(result, ttest_result, stringsAsFactors = F)
                # write.table(ttest_result, file=paste0(path, '/03_Diversity/Alpha/alpha_div_collated/', index, '/ttest.csv'), sep = ',', row.names = F)
            }else{
                ## wilcox
                ...
                # write.table(wilcox_result, file=paste0(path, '/03_Diversity/Alpha/alpha_div_collated/', index, '/wilcox.csv'), sep = ',', row.names = F)
            }
         }else{
            group_list <- unlist(strsplit(as.character(groupvs),"_vs_"))
            tmp_data <- data[data$Group %in% group_list,]
            normality_test <- shapiro.test(tmp_data$x)
            if (normality_test$p.value > 0.05){
                ## tukey
            ...
            ## 在单因素方差分析ANOVA中，如果该因素影响比较显著，那么需要进一步利用多重比较方法比较该因素不同水平的影响，确定不同水平下该因素的影响是否显著。常见的多重比较方法主要有两种，LSD法和Tukey HSD法, 无论是LSD还是Tukey进行多重比较，都需要先进行ANOVA分析
                ...
                write.table(tukey_result, file=paste0(path, '/03_Diversity/Alpha/alpha_div_collated/', index, '/', groupvs, '.csv'), sep = ',', quote = F)
            }else{
                ...
                wilcox <- kruskal(tmp_data$x, tmp_data$Group, group = F)
                wilcox_result <- wilcox$comparison
                wilcox_result$method <- 'kruskal'
                # result <- rbind(result, wilcox_result, stringsAsFactors = F)
                write.table(wilcox_result, file=paste0(path, '/03_Diversity/Alpha/alpha_div_collated/', index, '/', groupvs, '.csv'), sep = ',', quote = F)
            }
        }
    }
    write.table(result, file=paste0(path, '/03_Diversity/Alpha/alpha_div_collated/', index, '/result.csv'), sep = ',', row.names = F, quote = F)
}
...
#         ## 判断是否是正太分布
#         normality_test <- shapiro.test(data$x)   # P>0.05 :正态分布，P<0.05 :非正态分布
#         if (normality_test$p.value > 0.05){
#             ## t-test
...
#         normality_test <- shapiro.test(data$x)
#         if (normality_test$p.value > 0.05){
#             ## tukey
#         ## 最小显著差数检验法(LSD法)
#         ## Tukey氏固定差距检验法(Tukey HSD)
...
#             tukey <- TukeyHSD(fit)
#             tukey_result <- tukey$Group
#             write.table(tukey_result, file=paste0(path, '/03_Diversity/Alpha/alpha_div_collated/', index, '/tukey.csv'), sep = ',')
#         }
#         else{
#             ##Kurskal-Wallis检验是Wilcoxon方法（其实是Mann-Whitney检验）用于多个样本。当对两个样本进行比较的时候，Kurskal-Wallis检验与Mann-Whitney检验是等价的。
#             # wilcox <- kruskal.test(x ~ Group, data)
...
#             wilcox_result <- wilcox$comparison
#             write.table(wilcox_result, file=paste0(path, '/03_Diversity/Alpha/alpha_div_collated/', index, '/wilcox.csv'), sep = ',')
#         }
#         # ## 每俩组wilcox
...
# result_wilcox <- data.frame(groupvs = character(), p_value = character(), stringsAsFactors = F)
# for (i in 1:nrow(groupvs)) {
#     group1<-unlist(strsplit(as.character(groupvs[i,1]),"_vs_"))[1]
#     group2<-unlist(strsplit(as.character(groupvs[i,1]),"_vs_"))[2]
#     tmp_data <- data[which((data$Group == group1 | (data$Group == group2)),]
#     wilcox <- wilcox.test(x ~ Group, tmp_data)
#     p_value <- wilcox$p.value
#     wilcox_result <- data.frame(groupvs, p_value)
#     result_wilcox <- rbind(result_wilcox, wilcox_result, stringsAsFactors = F)
